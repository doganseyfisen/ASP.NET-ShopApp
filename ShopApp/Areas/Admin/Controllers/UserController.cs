...
namespace ShopApp.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
            ...
            return View(
                new UserDtoForCreation()
                {
                    Roles = new HashSet<string>(
                        _manager.AuthService.Roles.Select(role => role.Name).ToList()
                    )
                }
            );
        }
        ...
        {
            var user = await _manager.AuthService.GetSelectedUserForUpdate(id);
        ...
        {
            var result = await _manager.AuthService.DeleteSelectedUser(userDto.UserName);
